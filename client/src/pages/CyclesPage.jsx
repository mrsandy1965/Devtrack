...
import { projectsAPI, cyclesAPI, tasksAPI } from '../api/services';
import { IconPlus, IconTrash, IconCheck } from '../components/Icons';

const STATUS_COLOR = { upcoming: '#6b7280', active: '#10b981', completed: '#3b82f6' };

function BurndownBar({ percent, color }) {
  return (
...
export default function CyclesPage() {
  ...
  const [expanded, setExpanded] = useState(null);  // cycle._id
  const [detail, setDetail]     = useState({});    // { [cycleId]: { burndown, tasks } }
  const [loading, setLoading]   = useState(true);
  const [showModal, setShowModal] = useState(false);
  const [unassigned, setUnassigned] = useState([]);   // tasks not in any cycle
  ...
  const load = useCallback(async () => {
    ...
    setUnassigned(taskRes.data.tasks.filter((t) => !t.cycleId && t.status !== 'done'));
  }, [projectId]);

  useEffect(() => { load().finally(() => setLoading(false)); }, [load]);

  const expandCycle = async (cycleId) => {
  ...
  const deleteCycle = async (cycleId, e) => {
    e.stopPropagation();
  ...
  const addTaskToCycle = async (cycleId, taskId) => {
  ...
  const removeTaskFromCycle = async (cycleId, taskId) => {
