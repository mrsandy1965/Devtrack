...
import {
  DndContext, closestCenter, PointerSensor, useSensor, useSensors, DragOverlay, DragStartEvent, DragEndEvent
} from '@dnd-kit/core';
import {
...
function TaskCard({ task, onClick, isDragging }: { task: any; onClick: (task: any) => void; isDragging: boolean }) {
...
// ── Quick Add ──────────────────────────────────────────────────────────────────
function QuickAdd({ projectId, status, onAdd }: { projectId: string; status: string; onAdd: (task: any) => void }) {
...
// ── Board Column ───────────────────────────────────────────────────────────────
function BoardColumn({ status, tasks, projectId, onTaskClick, activeId, onAdd }: any) {
...
export default function BoardPage() {
  ...
  const handleDragEnd = async ({ active, over }: DragEndEvent) => {
    ...
    let srcCol: string | undefined, destCol: string | undefined;
    ...
    setBoard((prev) => {
      const next = { ...prev };
      const task = next[srcCol as string].find((t) => t._id === active.id);
      next[srcCol as string] = next[srcCol as string].filter((t) => t._id !== active.id);

      const destIdx = next[destCol as string].findIndex((t) => t._id === over.id);
      const insertAt = destIdx >= 0 ? destIdx : next[destCol as string].length;
      next[destCol as string] = [...next[destCol as string].slice(0, insertAt), { ...task, status: destCol }, ...next[destCol as string].slice(insertAt)];
      return next;
    });

    // Persist to API
    await tasksAPI.update(active.id as string, { status: destCol });
    const updatedTasks = board[destCol as string];
