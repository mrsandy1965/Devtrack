...
export default function TaskDetailPanel({ taskId, onClose, onUpdate }: TaskDetailPanelProps) {
  ...
  const update = async (patch: any) => {
    ...
    try {
      ...
      setData((prev: any) => ({ ...prev, task: res.data.task }));
  ...
  const submitComment = async (e: React.FormEvent) => {
    ...
    setData((prev: any) => ({ ...prev, comments: [...prev.comments, res.data.comment] }));
    setComment('');
  };

  const deleteComment = async (commentId: string) => {
    await tasksAPI.deleteComment(taskId, commentId);
    setData((prev: any) => ({ ...prev, comments: prev.comments.filter((c: any) => c._id !== commentId) }));
  };
